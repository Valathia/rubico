...
uint8_t go_to_next_corner(Cube* restrict cube_arr,Solution* sol) {
   const char* cmove[3] = {D_P,D2,D};
   for(uint8_t i=0;i<3;i++) {
      if(!check(cube_arr[0].f[DOWN],cube_orientation[DOWN],down_corners_mask[i])) {
...
uint8_t align_edges(Cube* restrict cube_arr,Solution* sol) {
   ...
   while(i<MAX_RECURSION) {
      ...
      #if DEBUGSOLVE3RDROW
         if(move) {
               printf("Moving Top row to align the most edges %d \n",move);
         }
      #endif
      ...
      if(is3rdRowEdgeAligned(cube_arr[0])) { //moving the up layer may solve the edges
         return 1;
      }

      apply_alg(cube_arr,sol,SONE);    //apply sone algorithm to swap adjacent edges on the front and left side
...
uint8_t align_corners(Cube* restrict cube_arr,Solution* sol) {
   ...
   while(!check_corner_alignment(&cube_arr[0],0,4) & (i<MAX_RECURSION)) {
      ...
      apply_alg(cube_arr,sol,NIKLAS);    //apply niklas algorithm to swap corners
      #if DEBUGALIGNCORNERS
         printf("\tAplied NIKLAS to Cube\n");
   ...
   return check_corner_alignment(&cube_arr[0],0,4);
}

//limit set by max recursion
uint8_t flip_corners(Cube* restrict cube_arr,Solution* sol) {

   corners_FLU_corner(cube_arr,sol,condition_corner_flip); //pre-move an unsolved corner to the FLU corner
   apply_alg(cube_arr,sol,ROT_Z_D);//flip the cube upside down move down to up
   uint8_t j = 0;
   
   while(j<MAX_RECURSION) {
      //is down face solved?
      if(check(cube_arr[0].f[cube_orientation[DOWN]],cube_orientation[DOWN],ALL)) {
         if(isSolved(cube_arr[0])) {    //is the cube solved?
               return 1;
         }
         else {
               
               uint8_t rot = is_row_rotated(&cube_arr[0],4,ROW1DONE);
      ...
      //#pragma GCC unroll ROT_SIZE
      for(int i=0; i<ROT_SIZE;i++) {
         get_corner(&cube_arr[0],DFR,out);
         //minimum is 2 times
         while(condition_corner_flip(out,DFR)) { //if flipped
               #if DEBUGFLIPCORNERS 
                  printf("  \t\t Flipping: \n");
                  printf("  \t\t  DFR: %s %s %s \n",colors[out[0]],colors[out[1]],colors[out[2]]);
               #endif

               apply_alg(cube_arr,sol,R_ALG R_ALG);
...
uint8_t solve_corners_1strow(Cube* restrict cube_arr,Solution* sol) {
   search_res out;
   uint8_t color = cube_orientation[DOWN];
   ...
   while(n && i<MAX_RECURSION) {
      ...
      */
      ...
      const char* cm = corner_move_table[out.ind][cmt_down_i(dest)][cmt_side_i(ori)];
...
uint8_t solve_3rd_row(Cube* restrict cube_arr,Solution* sol){
   ...
   */
   ...
   if(!isSolved(cube_arr[0])) {
      ...
      if(!check_corner_alignment(&cube_arr[0],0,4)) {
